...
|---|---|---|---|
| `liberu/theme-base` | shared | none | accessible layout, semantic tokens, cascade layers and progressive JavaScript primitives |
| `liberu/theme-default` | public | `liberu-base` | neutral light presentation |
| `liberu/theme-dark` | public | `liberu-base` | dark presentation through token overrides |
| `liberu/theme-clear-signal` | public | `liberu-base` | alternate high-clarity brand palette |
...
This repository is also the integration workspace for the bundled packages, so their authoritative package sources are already present at the installer destinations. It requires and exercises the installer itself without asking Composer to install a package over its own source directory. Release consumers require, for example, `liberu/theme-default` and `liberu/theme-base` from their package repositories.
