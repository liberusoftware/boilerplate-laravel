...
- The demo-shaped `searchPosts()`/`searchGroups()` leave for `search-demo`. `searchAll()` now
  iterates a `SearcherRegistry`, so a package owning a searchable concept contributes a type
  instead of this package naming it.
...
- Extract local search behavior and configurable model boundaries from the host.
