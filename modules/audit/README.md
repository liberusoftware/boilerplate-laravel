...
The trusted `liberu/composer-installer` places it in `/modules/audit`. The installed directory remains tracked by the host repository, while Composer and its lock file remain the source of version truth. Installation does not imply runtime enablement or commercial entitlement.
